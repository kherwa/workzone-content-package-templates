  ...
  i18n: {
    ...
    create: function (dir, basename, settings) {
      if (!settings || !settings.from || !settings.to) {
        console.log(
          "i18n: cannot create i18n file. No from/to languages given"
        );
        return;
      }
      var fromFile;
      if (settings.from === "") {
        fromFile = basename.replace(
          ".properties",
      ...
      for (var i = 0; i < settings.to.length; i++) {
        var toFile;
        if (settings.to[i] === "") {
          toFile = basename.replace(
            ".properties",
            settings.to[i] + ".properties"
          );
        } else {
          toFile = basename.replace(
            ".properties",
            "_" + settings.to[i] + ".properties"
          );
        }
    ...
    process: function (manifestpath, i18npath) {
      ...
      if (manifest["sap.app"]) {
        ...
        i18nsettings =
          manifest["sap.artifact"] && manifest["sap.artifact"].i18n;
      } else {
        console.log(
          "Warning: No sap.app, sap.package, sap.artifact i18n setting found. Are you processing the right file"
        );
        return;
      }
      ...
      if (i18nsettings && typeof i18nsettings === "string") {
        ...
        if (fs.pathExistsSync(i18n)) {
          util.i18n.create(i18n, basefilename, {
            from: "en_US",
            to: ["en", ""],
          });
          ...
          console.log(
            "Folder with file " +
              i18nsettings +
              " does not exist. Remove i18n setting or create a folder containing at least a file " +
              i18nsettings +
              " or " +
              i18nsettings.replace(".properties", "_en_US.properties")
          );
          throw new Error(
            "Folder with file " +
              i18nsettings +
    ...
    parseValue: function (sLine) {
      var rMatcher = /(\\u[0-9a-fA-F]{0,4})|(\\.)/g;
      /**
